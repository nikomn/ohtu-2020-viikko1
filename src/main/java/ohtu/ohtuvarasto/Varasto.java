...
public class Varasto {
    ...
    public Varasto(double tilavuus, double alkuSaldo) { // kuormitetaan
        ...
        double validoituAlkusaldo = tarkistaAlkusaldoEiNegatiivinen(alkuSaldo);
        alkuSaldo = tarkistaAlkusaldoEiLiianSuuri(validoituAlkusaldo);
    ...
    private double tarkistaAlkusaldoEiNegatiivinen(double s) {
        if (s > 0.0) {
            return s;
        } else {
            return 0.0;
    ...
    private double tarkistaAlkusaldoEiLiianSuuri(double s) {
        if (s >= 0.0 && s <= tilavuus) {
            return s;
        } else {
            return tilavuus;
